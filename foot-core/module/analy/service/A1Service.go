...
func (this *A1Service) analyStub(v *pojo.MatchLast) (int, *entity5.AnalyResult) {
	//base.Log.Info("A1Service analyStub")
	...
	if len(eList) < 3 {
		//base.Log.Info("亚赔  len(eList) < 3  ", matchId)
		return -1, temp_data
	}
	for _, ev := range eList {
		if strings.EqualFold(strconv.Itoa(ev.CompId), "81") {
			e81 = ev
			continue
		}
		if strings.EqualFold(strconv.Itoa(ev.CompId), "616") {
			e616 = ev
			continue
		}
	...
	if e81 == nil || e616 == nil {
		//base.Log.Info("亚赔  e81 == nil || e616 == nil ", matchId)
		return -1, temp_data
	}
	...
	if len(aList) < 1 {
		//base.Log.Info("亚赔 18Bet is not find ", aList)
		return -1, temp_data
	}
	a18Bet = aList[0]
	temp_data.LetBall = a18Bet.EPanKou
	if math.Abs(a18Bet.EPanKou) > this.MaxLetBall {
		//base.Log.Info(" math.Abs(a18Bet.EPanKou) > this.MaxLetBall ", matchId)
		//temp_data.Result = ""
		return -2, temp_data
	}
	...
	if euroDirection == 3 && asiaDirection == 0 {
		...
		//base.Log.Info("亚赔 preResult: is not sure . ", "  ", matchId)
		return -3, temp_data
	}

	//base.Log.Info("亚赔 preResult: ", preResult, "  ", matchId)
