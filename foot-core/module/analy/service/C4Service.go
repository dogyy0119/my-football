...
func (this *C4Service) analyStub(v *pojo.MatchLast) (int, *entity5.AnalyResult) {
	...
	err1 := this.BFJinService.PageSql("SELECT j.* FROM t_b_f_jin j WHERE j.SclassID = "+v.LeagueId+" AND (j.HomeTeam = '"+v.MainTeamId+"' OR j.GuestTeam = '"+v.MainTeamId+"') AND STR_TO_DATE(j.MatchTimeStr, '%Y%m%d%H%i%s') <  '"+matchDateStr+"' ORDER BY j.MatchTimeStr DESC ", page, &mainJinList)
	if nil != err1 {
		return -2, temp_data
	}

	guestJinList := make([]*pojo.BFJin, 0)
	err2 := this.BFJinService.PageSql("SELECT j.* FROM t_b_f_jin j WHERE j.SclassID = "+v.LeagueId+" AND (j.HomeTeam = '"+v.GuestTeamId+"' OR j.GuestTeam = '"+v.GuestTeamId+"') AND STR_TO_DATE(j.MatchTimeStr, '%Y%m%d%H%i%s') <  '"+matchDateStr+"' ORDER BY j.MatchTimeStr DESC ", page, &guestJinList)
	if nil != err2 {
		return -2, temp_data
	}
	...
	temp_data.SLetBall = a18Bet.SPanKou
	temp_data.LetBall = a18Bet.EPanKou
	...
	//限制初盘,即时盘让球在0.25以内
	range_letball := math.Abs(a18Bet.SPanKou - a18Bet.EPanKou)
	if (a18Bet.SPanKou > 0 && a18Bet.EPanKou < 0) || (a18Bet.SPanKou < 0 && a18Bet.EPanKou > 0) {
		temp_data.TOVoidDesc = fmt.Sprintf("Turn:%v,%v,%v", strconv.FormatFloat(range_letball, 'f', -1, 64), strconv.FormatFloat(a18Bet.SPanKou, 'f', -1, 64), strconv.FormatFloat(a18Bet.EPanKou, 'f', -1, 64))
		temp_data.PreResult = -1
		temp_data.HitCount = 3
		return -2, temp_data
	}
	if math.Abs(a18Bet.SPanKou-a18Bet.EPanKou) > 0.25 {
		temp_data.TOVoidDesc = fmt.Sprintf("Change:%v,%v,%v", strconv.FormatFloat(range_letball, 'f', -1, 64), strconv.FormatFloat(a18Bet.SPanKou, 'f', -1, 64), strconv.FormatFloat(a18Bet.EPanKou, 'f', -1, 64))
		temp_data.PreResult = -1
		temp_data.HitCount = 3
		return -2, temp_data
	}
	...
	base.Log.Info(this.ModelName()+"比赛时间:", matchDateStr+",对阵:"+v.GuestTeamId, ",初盘让球:", a18Bet.SPanKou, ",即时盘让球:", eLetBall, ",球差:", diffGoal, ",分差:", diffScore, " ,比分:", v.MainTeamGoals, ":", v.GuestTeamGoals, " ,半场比分:", v.MainTeamHalfGoals, ":", v.GuestTeamHalfGoals)
